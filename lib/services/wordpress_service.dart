...
class WordPressService {
  ...
  Future<List<Article>> fetchPostsByTagForToolsAndWidget(
      int pageNumber, String tagID, int contentAmount) async {
    final String url =
        '${WpConfig.baseURL}/wp-json/wp/v2/posts/?tags=$tagID&page=$pageNumber&per_page=$contentAmount';
    List<Article> articles = [];
    try {
  ...
  Future<List<Category>> fetchCategoriesByIDs(List ids) async {
    ...
    if (ids.isEmpty) {
      ...
      final String url =
          '${WpConfig.baseURL}/wp-json/wp/v2/categories?include=$categoryIds&orderby=include';
      try {
        final response = await http.get(Uri.parse(url));
        if (response.statusCode == 200) {
  ...
  Future<Sidemenu> getSidemenu() async {
    final response = await http.get(
      Uri.parse('https://api.arynews.tv/v3/sidemenuv3.json'),
    );
